...
"""
    ...
    try:
        ...
        # We start with one item: "Hat"

        item1 = item_create_request.ItemCreateRequest(id="35")  # Hat
        ...
        logger.info("Current Total: %s cents", checkout_data["totals"][-1]["amount"])
        ...
        logger.info("\nSTEP 2: Adding a second item (Jacket)...")

        # Update Item 1 (Hat) - Keep quantity 1
        ...
        li_1 = next(
            li for li in checkout_data["line_items"] if li["item"]["id"] == "35"  # Hat
        )
        ...
        item1_update = item_update_request.ItemUpdateRequest(id="35")
        ...
        item2_update = item_update_request.ItemUpdateRequest(id="31")
        ...
        if not checkout_data.get("fulfillment") or not checkout_data["fulfillment"].get(
            "methods"
        ):
            ...
            trigger_req = checkout_update_request.CheckoutUpdateRequest(
                ...
                fulfillment={
                    "methods": [
                        {
                            "id": "placeholder-shipping-method-id",
                            "type": "shipping",
        ...
        if checkout_data.get("fulfillment") and checkout_data["fulfillment"].get(
            ...
            if method.get("destinations"):
                ...
                trigger_req.fulfillment = {
                    "methods": [
                        {
                            "id": method.get("id", "placeholder-id"),
                            "type": "shipping",
                            "selected_destination_id": dest_id,
                            "line_item_ids": method.get(
                                "line_item_ids", [li_1["id"], li_2["id"]]
                            ),
                        }
                    ]
                        ...
                        "methods": [
                            {
                                ...
                                "groups": [
                                    {
                                        "id": method["groups"][0].get(
                                            "id", "placeholder-group-id"
                                        ),
                                        "line_item_ids": method["groups"][0].get(
                                            "line_item_ids", [li_1["id"], li_2["id"]]
                                        ),
        ...
        if not any(h["id"] == target_handler for h in supported_handlers):
            logger.warning(
                "Merchant does not list support for %s, but we will try anyway.",
                target_handler,
            )
